...
module interest_tokens::sui_yield {
  use std::ascii;
  use std::option;
  ...
  use interest_framework::math::{fdiv, fmul};
  ...
  fun init(witness: SUI_YIELD, ctx: &mut TxContext) {
    let (treasury_cap, metadata) = sft::create_sft(
      ...
      option::some(url::new_unsafe_from_bytes(b"https://interestprotocol.infura-ipfs.io/ipfs/QmcVgNciMdAqVzJ8uZUgibn2gvhMtKRdG6J2qNrB37LRFf")),
      ctx
    );
  ...
  public fun expire(storage: &mut SuiYieldStorage, publisher: &Publisher, token: SuiYield, ctx: &mut TxContext): SuiYield {
    assert!(is_minter(storage, object::id(publisher)), EInvalidMinter);
  ...
  entry public fun add_minter(_: &AdminCap, storage: &mut SuiYieldStorage, id: ID) {
  ...
  entry public fun remove_minter(_: &AdminCap, storage: &mut SuiYieldStorage, id: ID) {
  ...
  public fun is_minter(storage: &SuiYieldStorage, id: ID): bool {
