...
module interest_tokens::soulbound_token {
  ...
  use interest_framework::constants::ten_years_epochs;

  // Errors
  const ELockPeriodTooLong: u64 = 0;
  const EAssetsStillLocked: u64 = 1;
  ...
  */
  public fun lock_asset<Asset: key + store>(sbt: &mut InterestSBT, asset: Asset, number_of_epochs: u64, ctx: &mut TxContext) {
    ...
    assert!(ten_years_epochs() >= number_of_epochs, ELockPeriodTooLong);
  ...
  */
  public fun unlock_asset<Asset: key + store>(sbt: &mut InterestSBT, ctx: &mut TxContext): vector<Asset> {
    ...
    assert!(unlock_epoch < tx_context::epoch(ctx), EAssetsStillLocked);
    assets
  } 
